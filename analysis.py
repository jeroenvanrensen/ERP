...
# plt.xlabel("Tape-afstand $T$ (cm)")
# plt.ylabel("Grootte vierkant $G$ (px)")
# plt.legend(loc=1, title="Led-afstand")
# plt.show()
...
plt.plot(xplot, func(par_best, xplot), "r-", linewidth=2, c="tab:blue")
...
xplot = np.linspace(np.min(x1s), np.max(x1s), num=1500)
plt.plot(xplot, func(par_best, xplot), "r-", linewidth=2, c="tab:orange")

plt.errorbar(
    x1s, y1s, yerr=y1_errors, xerr=x1_errors, fmt=".", c="tab:blue", label="1 laag"
)
plt.errorbar(
    x2s, y2s, yerr=y2_errors, xerr=x2_errors, fmt=".", c="tab:orange", label="2 lagen"
)
plt.xlabel("Tape-led-ratio $T/L$ (AU)")
plt.ylabel("Grootte vierkant $G$ (px)")
plt.legend(title="Lagen tape")
plt.show()
